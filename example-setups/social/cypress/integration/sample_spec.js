const HASURA_QUERY_URL="https://hasura-6khqdzs.rocketgraph.app/v1/query"

Cypress.Cookies.debug(true) // now Cypress will log when it alters cookies
Cypress.Cookies.defaults({
    preserve: ["jwt", "user_id"],
})
...
describe('Creates the todos table on the Hasura endpoint', () => {
    it('Creates the todos table', () => {
        cy.request({
            method: 'POST', 
            url: "https://hasura-6khqdzs.rocketgraph.app/v1/query",
            headers: {
...
describe('Creates a user', () => {
    const signup = 'https://backend-6khqdzs.rocketgraph.app/api/signup';
    beforeEach(() => {
        cy.visit('http://localhost:3000/signup')
    })
    it('Creates a user with role User', () => {
        ...
        cy.get('#inputUserPassword')
            .should('be.visible')
            .type('test')
        cy.intercept(signup).as("doSignup")
        cy.get('#signupButton')
            .should('be.visible')
            .click()
        cy.wait(["@doSignup"])
...
describe('Creates todos and check that they are visible', () => {
    const query = "https://hasura-6khqdzs.rocketgraph.app/v1/graphql"
    beforeEach(() => {
